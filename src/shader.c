...
void
shader_set_integer(shader_t *shader, const char *name, int value) {
   glUniform1i(glGetUniformLocation(shader->ID, name), value);
}

void
shader_set_vector2pf(shader_t *shader, const char *name, float x, float y) {
   glUniform2f(glGetUniformLocation(shader->ID, name), x, y);
}

void
shader_set_vector2f(shader_t *shader, const char *name, vec2 value) {
    glUniform2f(glGetUniformLocation(shader->ID, name), value[0], value[1]);
}

void
shader_set_vector3pf(shader_t *shader, const char *name, float x, float y, float z) {
   glUniform3f(glGetUniformLocation(shader->ID, name), x, y, z);
}

void
shader_set_vector3f(shader_t *shader, const char *name, vec3 value) {
   glUniform3f(glGetUniformLocation(shader->ID, name), value[0], value[1], value[2]);
}

void
shader_set_vector4pf(shader_t *shader, const char *name, float x, float y, float z, float w) {
   glUniform4f(glGetUniformLocation(shader->ID, name), x, y, z, w);
}

void
shader_set_vector4f(shader_t *shader, const char *name, vec4 value) {
    glUniform4f(glGetUniformLocation(shader->ID, name), value[0], value[1], value[2], value[3]);
}

void
shader_set_matrix4(shader_t *shader, const char *name, mat4 matrix) {
